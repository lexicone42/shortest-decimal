/-
  Interval/IntervalRNA.lean

  The acceptance interval for round-to-nearest-ties-to-away (RNA).
  Same midpoint boundaries as RNE but with different inclusivity:
  - For positive x: lower inclusive, upper exclusive
  - For negative x: lower exclusive, upper inclusive
  This reflects RNA's tie-breaking: away from zero.
-/
...
/-- The RNA acceptance interval.
    Same bounds as the RNE interval (midpoints between neighbors),
    but inclusivity follows RNA tie-breaking:
    positive x → lower inclusive, upper exclusive
    negative x → lower exclusive, upper inclusive -/
def rnaInterval (x : F64) (_hfin : x.isFinite) : AcceptanceInterval :=
...
private theorem rna_effSig_pos (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
...
private theorem rna_scaleN_pos_and_lt {u w : Nat} (hu : 0 < u) (hw : u < w) (e2 : Int) :
...
theorem rna_abs_interval_pos (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
    let iv := rnaInterval x hfin
  ...
  simp only []
  have hmf := rna_effSig_pos x hfin hne
  ...
  have hw_gt : 4 * x.effectiveSignificand -
    ...
    split <;> omega
  have key := rna_scaleN_pos_and_lt hu_pos hw_gt (x.effectiveBinaryExp - 2)
  cases hs : x.sign
  · unfold rnaInterval; simp only [hs, Bool.false_eq_true, ite_false]; exact key
  · unfold rnaInterval; simp only [hs, Bool.true_eq_false, ite_false, ite_true, neg_neg]; exact key

/-! ## Core rounding lemma for RNA -/

/-- Rounding in [n - 1/2, n + 1/2) with RNA tie-break gives n. -/
private theorem round_rna_in_half_interval (n : ℕ) (hn : n ≥ 1) (sigExact : ℚ)
  ...
  · push_neg at hge
    ...
    rw [htonat]
    rw [if_neg (by rw [hn_cast]; linarith)]
    omega

/-! ## Decomposing roundSignificand_rna -/

private def sigRoundedOf_rna (qAbs : ℚ) (bexp : Nat) : Nat :=
...
private def branchOf_rna (sigRounded : Nat) (bexp : Nat) : Nat × Bool :=
  if bexp = 0 then
...
private theorem roundSignificand_rna_split (qAbs : ℚ) (bexp : Nat) :
    F64.roundSignificand_rna qAbs bexp = branchOf_rna (sigRoundedOf_rna qAbs bexp) bexp := rfl

private theorem branchOf_rna_mf (x : F64) :
    branchOf_rna x.effectiveSignificand x.biasedExp.val = (x.mantissa.val, false) := by
  unfold branchOf_rna
...
private theorem rna_pow_ef_cancel (ef : Int) (hef2 : ¬(ef - 2 ≥ 0)) (hef : ef ≥ 0) :
    (2:ℚ) ^ (-(ef - 2)).toNat * 2 ^ ef.toNat = 4 := by
  rw [show (4:ℚ) = 2^2 from by norm_num, ← pow_add]; congr 1; omega

private theorem rna_pow_ef_cancel_neg (ef : Int) (hef : ef < 0) :
    (2:ℚ) ^ (-ef).toNat * 4 = 2 ^ (-(ef - 2)).toNat := by
  rw [show (4:ℚ) = 2^2 from by norm_num, ← pow_add]; congr 1; omega

private theorem rna_sigExact_lower_bound (n : ℕ) (qAbs : ℚ) (ef : Int)
    ...
    · simp only [show ¬(ef - 2 ≥ 0) from by omega, ↓reduceIte, if_pos hef] at hscale ⊢
      ...
      linarith [mul_le_mul_of_nonneg_right hle (show (0:ℚ) ≤ 2 ^ ef.toNat from by positivity),
                show qAbs * (2:ℚ) ^ (-(ef - 2)).toNat * 2 ^ ef.toNat = qAbs * 4 from by
                  rw [mul_assoc, rna_pow_ef_cancel ef (by omega) hef]]
    · push_neg at hef
      ...
      linarith [show qAbs * (2:ℚ) ^ (-ef).toNat * 4 = qAbs * 2 ^ (-(ef - 2)).toNat from by
                  rw [mul_assoc, rna_pow_ef_cancel_neg ef (by omega)]]

private theorem rna_sigExact_upper_bound_strict (n : ℕ) (qAbs : ℚ) (ef : Int)
...
/-- sigRoundedOf_rna = effectiveSignificand when sigExact ∈ [mf - 1/2, mf + 1/2). -/
private theorem sigRoundedOf_rna_from_bounds (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (qAbs : ℚ)
    ...
    sigRoundedOf_rna qAbs x.biasedExp.val = x.effectiveSignificand := by
  unfold sigRoundedOf_rna; simp only []; rw [F64.binExp_eq]
  exact round_rna_in_half_interval x.effectiveSignificand
    (rna_effSig_pos x hfin hne) _ hsig_lo hsig_hi

/-! ## Interval soundness -/

/-! ### Extracting |q| bounds from the interval -/
...
/-- Extract weak |q| bounds from rnaInterval (≤ on both sides). -/
private theorem rna_interval_abs_bounds_weak (x : F64) (hfin : x.isFinite)
    (q : ℚ) (hq_ne : q ≠ 0)
    (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q) :
    let mf := x.effectiveSignificand
    let e2 := x.effectiveBinaryExp - 2
  ...
  simp only []
  unfold AcceptanceInterval.contains rnaInterval at hq; simp only [] at hq
  cases hs : x.sign
  · have hq_pos := pos_of_sign_false' q hq_ne hdec hs
    ...
    · by_cases he2 : x.effectiveBinaryExp - 2 ≥ 0
      · simp only [if_pos he2] at hlo ⊢; exact hlo
      · simp only [if_neg he2] at hlo ⊢; exact hlo
    · by_cases he2 : x.effectiveBinaryExp - 2 ≥ 0
      · simp only [if_pos he2] at hhi ⊢; linarith
      · simp only [if_neg he2] at hhi ⊢; linarith
  · have hq_neg := neg_of_sign_true' q hdec hs
    rw [abs_of_neg hq_neg]
    simp only [hs, Bool.false_eq_true, Bool.true_eq_false, ite_true, ite_false,
    ...
    · by_cases he2 : x.effectiveBinaryExp - 2 ≥ 0
      ...
      · simp only [if_neg he2] at hhi ⊢; linarith
    · by_cases he2 : x.effectiveBinaryExp - 2 ≥ 0
      · simp only [if_pos he2] at hlo ⊢; linarith
      · simp only [if_neg he2] at hlo ⊢; linarith

/-- For RNA, the upper bound on |q| (in terms of w = 4mf+2) is always strict. -/
private theorem rna_abs_upper_strict (x : F64) (hfin : x.isFinite)
    (q : ℚ) (hq_ne : q ≠ 0)
    (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q) :
    let e2 := x.effectiveBinaryExp - 2
    let w := 4 * x.effectiveSignificand + 2
    |q| < (if e2 ≥ 0 then (w : ℚ) * 2 ^ e2.toNat else (w : ℚ) / 2 ^ (-e2).toNat) := by
  simp only []
  unfold AcceptanceInterval.contains rnaInterval at hq; simp only [] at hq
  cases hs : x.sign
  · have hq_pos := pos_of_sign_false' q hq_ne hdec hs
    ...
    obtain ⟨_, hhi⟩ := hq
    by_cases he2 : x.effectiveBinaryExp - 2 ≥ 0
    · simp only [if_pos he2] at hhi ⊢; exact hhi
    · simp only [if_neg he2] at hhi ⊢; exact hhi
  · have hq_neg := neg_of_sign_true' q hdec hs
    rw [abs_of_neg hq_neg]
    simp only [hs, Bool.false_eq_true, Bool.true_eq_false, ite_true, ite_false,
               decide_eq_false_iff_not, decide_eq_true_eq] at hq
...
private theorem rna_round_same_exp (x : F64) (hfin : x.isFinite)
    (hne : x.toRat ≠ 0) (q : ℚ) (hq_ne : q ≠ 0)
    (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe : F64.findBiasedExp |q| = x.biasedExp.val) :
    F64.roundTiesAway q = x := by
  have hexp_lt := F64.finite_implies_exp_lt x hfin
  unfold F64.roundTiesAway
  rw [if_neg hq_ne]
  simp only []
  rw [hfbe]
  have hrs : F64.roundSignificand_rna |q| x.biasedExp.val = (x.mantissa.val, false) := by
    rw [roundSignificand_rna_split]
    ...
    set u := 4 * mf - delta
    set w := 4 * mf + 2
    have hmf_pos := rna_effSig_pos x hfin hne
    have hdelta_le : delta ≤ 2 := by
      show (if x.mantissa.val = 0 ∧ x.biasedExp.val > 1 then 1 else 2 : Nat) ≤ 2; split <;> omega
    have ⟨habs_lo, _⟩ := rna_interval_abs_bounds_weak x hfin q hq_ne hdec hq
    have habs_hi_strict := rna_abs_upper_strict x hfin q hq_ne hdec hq
    have hsig_lo := rna_sigExact_lower_bound u |q| ef habs_lo
    have hsig_hi := rna_sigExact_upper_bound_strict w |q| ef habs_hi_strict
    ...
    have hsig := sigRoundedOf_rna_from_bounds x hfin hne |q|
      (le_trans hu_lower hsig_lo) (by rw [← hw_eq]; exact hsig_hi)
    rw [hsig]
    exact branchOf_rna_mf x
  rw [hrs]
...
-- Reproduce threshold helpers that are private in Interval.lean

private theorem rna_scaleN_ge_threshQ (u : Nat) (be : Nat) (hu : u ≥ 2^54)
...
private theorem rna_scaleN_lt_threshQ_succ (w : Nat) (be : Nat) (hw : (w : ℚ) < 2^55)
    (hbe_ge : be ≥ 1) (hbe_lt : be < 2047) :
    let e2 : Int := (be : Int) - 1077
...
private theorem rna_boundary_bexp_ge_one (x : F64) (hfin : x.isFinite) (_hne : x.toRat ≠ 0)
  ...
  have ⟨_, habs_hi⟩ := rna_interval_abs_bounds_weak x hfin q hq_ne hdec hq
...
private theorem rna_boundary_mant_zero (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (q : ℚ) (hq_ne : q ≠ 0) (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
    x.mantissa.val = 0 := by
  have hbexp := rna_boundary_bexp_ge_one x hfin hne q hq_ne hdec hq hfbe_ne
  by_contra hmant_ne
  ...
  have ⟨habs_lo, habs_hi⟩ := rna_interval_abs_bounds_weak x hfin q hq_ne hdec hq
  simp only [hdelta] at habs_lo
  rw [he2] at habs_lo habs_hi
  have hscaleN_ge := rna_scaleN_ge_threshQ (4 * x.effectiveSignificand - 2) x.biasedExp.val
  ...
  have hw_lt : (4 * x.effectiveSignificand + 2 : ℚ) < 2^55 := by
    ...
    linarith
  have hlt_thresh := rna_scaleN_lt_threshQ_succ (4 * x.effectiveSignificand + 2) x.biasedExp.val
...
private theorem rna_boundary_qabs_lt_thresh (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (q : ℚ) (hq_ne : q ≠ 0) (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
  ...
  have hbexp := rna_boundary_bexp_ge_one x hfin hne q hq_ne hdec hq hfbe_ne
  have hmant := rna_boundary_mant_zero x hfin hne q hq_ne hdec hq hfbe_ne
  have hexp_lt := F64.finite_implies_exp_lt x hfin
  ...
  have hlt_thresh := rna_scaleN_lt_threshQ_succ (4 * 2^52 + 2) x.biasedExp.val hw_lt hbexp hexp_lt
...
private theorem rna_boundary_qabs_ge_prev (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (q : ℚ) (hq_ne : q ≠ 0) (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
...
private theorem rna_boundary_fbe_eq (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (q : ℚ) (hq_ne : q ≠ 0) (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
    F64.findBiasedExp |q| = x.biasedExp.val - 1 := by
  have hbexp := rna_boundary_bexp_ge_one x hfin hne q hq_ne hdec hq hfbe_ne
  have hqlt := rna_boundary_qabs_lt_thresh x hfin hne q hq_ne hdec hq hfbe_ne
  have hqge := rna_boundary_qabs_ge_prev x hfin hne q hq_ne hdec hq hfbe_ne
  have hexp_lt := F64.finite_implies_exp_lt x hfin
  unfold F64.findBiasedExp
  apply F64.go_eq
  · omega
...
/-! #### Step 3: roundSignificand_rna(|q|, biasedExp - 1) = (0, true) -/

/-- Helper: if sigExact ∈ [N - 1/2, N) with N ≥ 2, then RNA rounding gives N. -/
private theorem round_rna_to_N (N : Nat) (sigExact : ℚ)
...
/-- RNA version of sigRoundedOf_eq_of_bounds. -/
private theorem sigRoundedOf_rna_eq_of_bounds (qAbs : ℚ) (bexp N : Nat)
    (hN_ge : N ≥ 2)
    (hlo : (N : ℚ) - 1/2 ≤
      let binExp : Int := if bexp = 0 then -1074 else (bexp : Int) - 1075
    ...
    sigRoundedOf_rna qAbs bexp = N := by
  unfold sigRoundedOf_rna
  simp only [] at hlo hhi ⊢
  exact round_rna_to_N N _ hlo hhi hN_ge

-- Reproduce the boundary sigExact conversion lemmas

private theorem rna_boundary_sigExact_lo_sub (qAbs : ℚ)
...
private theorem rna_boundary_sigExact_hi_sub (qAbs : ℚ)
...
private theorem rna_boundary_sigExact_lo_norm (bexp : Nat) (qAbs : ℚ) (hbexp : bexp ≥ 2)
...
private theorem rna_boundary_sigExact_hi_norm (bexp : Nat) (qAbs : ℚ) (hbexp : bexp ≥ 2)
    (hexp_lt : bexp < 2047)
...
private theorem rna_boundary_roundSig (x : F64) (hfin : x.isFinite) (hne : x.toRat ≠ 0)
    (q : ℚ) (hq_ne : q ≠ 0) (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
    F64.roundSignificand_rna |q| (x.biasedExp.val - 1) = (0, true) := by
  have hbexp := rna_boundary_bexp_ge_one x hfin hne q hq_ne hdec hq hfbe_ne
  have hmant := rna_boundary_mant_zero x hfin hne q hq_ne hdec hq hfbe_ne
  have hqlt := rna_boundary_qabs_lt_thresh x hfin hne q hq_ne hdec hq hfbe_ne
  have hexp_lt := F64.finite_implies_exp_lt x hfin
  ...
  · simp only [hbe1, show 1 - 1 = 0 from by omega]
    suffices hsig : sigRoundedOf_rna |q| 0 = 2^52 by
      rw [hsig]; unfold branchOf_rna; simp
    have he2_val : (x.biasedExp.val : Int) - 1077 = -1076 := by omega
    ...
    exact sigRoundedOf_rna_eq_of_bounds |q| 0 (2^52) (by norm_num)
      (by simp only [↓reduceIte, show ¬((-1074 : Int) ≥ 0) from by omega,
                      show ((-(-1074 : Int)).toNat) = 1074 from by omega]
          exact rna_boundary_sigExact_lo_sub |q| habs_lo)
      (by simp only [↓reduceIte, show ¬((-1074 : Int) ≥ 0) from by omega,
                      show ((-(-1074 : Int)).toNat) = 1074 from by omega]
          exact rna_boundary_sigExact_hi_sub |q| hqlt)
  · have hbe_ge2 : x.biasedExp.val ≥ 2 := by omega
    have hbm1_ne0 : x.biasedExp.val - 1 ≠ 0 := by omega
    suffices hsig : sigRoundedOf_rna |q| (x.biasedExp.val - 1) = 2^53 by
      rw [hsig]; unfold branchOf_rna; simp only [hbm1_ne0, ↓reduceIte]; norm_num
    ...
    exact sigRoundedOf_rna_eq_of_bounds |q| (x.biasedExp.val - 1) (2^53) (by norm_num)
      (by simp only [hbm1_ne0, ↓reduceIte, hbinexp]
          exact rna_boundary_sigExact_lo_norm x.biasedExp.val |q| hbe_ge2 habs_lo)
      (by simp only [hbm1_ne0, ↓reduceIte, hbinexp]
          exact rna_boundary_sigExact_hi_norm x.biasedExp.val |q| hbe_ge2 hexp_lt hqlt)

/-! #### Step 4: Assembly -/

private theorem rna_round_boundary (x : F64) (hfin : x.isFinite)
    (hne : x.toRat ≠ 0) (q : ℚ) (hq_ne : q ≠ 0)
    (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q)
    (hfbe_ne : F64.findBiasedExp |q| ≠ x.biasedExp.val) :
  ...
  have hbexp := rna_boundary_bexp_ge_one x hfin hne q hq_ne hdec hq hfbe_ne
  have hmant := rna_boundary_mant_zero x hfin hne q hq_ne hdec hq hfbe_ne
  have hfbe := rna_boundary_fbe_eq x hfin hne q hq_ne hdec hq hfbe_ne
  have hrs := rna_boundary_roundSig x hfin hne q hq_ne hdec hq hfbe_ne
  have hexp_lt := F64.finite_implies_exp_lt x hfin
  unfold F64.roundTiesAway
  rw [if_neg hq_ne]
  simp only []
...
private theorem rna_round_core (x : F64) (hfin : x.isFinite)
    (hne : x.toRat ≠ 0) (q : ℚ) (hq_ne : q ≠ 0)
    (hdec : decide (q < 0) = x.sign)
    (hq : (rnaInterval x hfin).contains q) :
    F64.roundTiesAway q = x := by
  by_cases hfbe : F64.findBiasedExp |q| = x.biasedExp.val
  · exact rna_round_same_exp x hfin hne q hq_ne hdec hq hfbe
  · exact rna_round_boundary x hfin hne q hq_ne hdec hq hfbe

/-- The RNA acceptance interval is sound: any rational in the interval
    rounds to x under round-to-nearest-ties-to-away. -/
theorem rna_interval_correct (x : F64) (hfin : x.isFinite)
    (hne : x.toRat ≠ 0)
    (q : ℚ) (hq : (rnaInterval x hfin).contains q) :
    F64.roundTiesAway q = x := by
  have hq_ne : q ≠ 0 := by
    intro heq; subst heq
    have ⟨hlo_pos, _⟩ := rna_abs_interval_pos x hfin hne
  ...
  exact rna_round_core x hfin hne q hq_ne hdec hq
