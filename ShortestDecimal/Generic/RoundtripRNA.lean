/-
  Generic/RoundtripRNA.lean

  THE GENERIC RNA ROUNDTRIP THEOREM: any DecimalConversionAlgorithmRNA
...
/-- Convert a Decimal to F64 via round-to-nearest-ties-to-away.
    For zero digits, construct the F64 directly (preserving sign). -/
def Decimal.toF64_rna (d : Decimal) : F64 :=
  if d.digits = 0 then ⟨d.sign, ⟨0, by omega⟩, ⟨0, by omega⟩⟩
  else F64.roundTiesAway d.toRat
...
/-! ## The Generic RNA Roundtrip Theorem -/

/-- THE GENERIC RNA ROUNDTRIP THEOREM.

    For ANY decimal conversion algorithm satisfying the RNA interface,
    and for ANY finite IEEE 754 double x:
      parse(format(algorithm(x))).toF64_rna roundtrips to x. -/
theorem generic_full_roundtrip_rna (alg : DecimalConversionAlgorithmRNA)
    (x : F64) (hfin : x.isFinite) :
    (Decimal.parse (Decimal.format (alg.convert x hfin))).map Decimal.toF64_rna = some x := by
  ...
  · -- Zero case
    ...
    simp [Option.map, Decimal.toF64_rna, hd0]
  ...
  · -- Non-zero case
    ...
    have hd_ne : (alg.convert x hfin).digits ≠ 0 := by
      ...
      rw [this] at h_in
      have ⟨hlo, _⟩ := ShortestDecimal.rna_abs_interval_pos x hfin h0
    ...
    simp [Decimal.toF64_rna, hd_ne]
    exact ShortestDecimal.rna_interval_correct x hfin h0 _ h_in

/-- Corollary: the algorithm's Decimal, converted to F64 via RNA, gives x. -/
theorem generic_decimal_roundtrip_rna (alg : DecimalConversionAlgorithmRNA)
    (x : F64) (hfin : x.isFinite) :
    Decimal.toF64_rna (alg.convert x hfin) = x := by
  have h := generic_full_roundtrip_rna alg x hfin
