/-
  IEEE754/RoundProof32.lean

  Proof of the projection property:
    roundToNearestEven(toRat(x)) = x  for all finite non-zero F32 x
-/
...
/-- For a finite F32, effectiveSignificand is bounded. -/
theorem effectiveSignificand_lt (x : F32) (_hfin : x.isFinite) :
    x.effectiveSignificand < 2^24 := by
  unfold effectiveSignificand
  split
  · exact Nat.lt_trans x.mantissa.isLt (by omega)
  · have := x.mantissa.isLt
    show 2 ^ mantBits + x.mantissa.val < 2 ^ 24
    simp [mantBits]; omega
...
private theorem effectiveSig_ne_zero (x : F32) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
...
theorem toRat_lt_zero_iff (x : F32) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
...
theorem mkFinite_eq (s : Bool) (e : Fin 256) (m : Fin (2^23))
    (he : e.val < 255) :
    mkFinite s e.val m.val = some ⟨s, ⟨e.val, by omega⟩, ⟨m.val, m.isLt⟩⟩ := by
  unfold mkFinite
  simp only [show e.val < 255 from he, show m.val < 2 ^ 23 from m.isLt,
             dite_true]

/-! ## |toRat| characterization -/

theorem toRat_abs (x : F32) (hfin : x.isFinite) :
    |x.toRat| =
...
def threshQ (e : Nat) : ℚ :=
  if e ≥ 127 then (2 ^ (e - 127) : ℚ) else 1 / (2 ^ (127 - e) : ℚ)

theorem threshQ_le_of_le {a b : Nat} (h : a ≤ b) : threshQ a ≤ threshQ b := by
  rcases eq_or_lt_of_le h with rfl | hlt
  · exact le_rfl
  · unfold threshQ
    by_cases ha : a ≥ 127 <;> by_cases hb : b ≥ 127
    · simp only [show a ≥ 127 from ha, show b ≥ 127 from hb, ↓reduceIte]
      exact_mod_cast Nat.pow_le_pow_right (by omega) (by omega)
    · omega
    · push_neg at ha
      simp only [show ¬(a ≥ 127) from by omega, show b ≥ 127 from hb, ↓reduceIte]
      calc 1 / (2 : ℚ) ^ (127 - a)
          ≤ 1 := by rw [one_div]; exact inv_le_one_of_one_le₀
                      (by exact_mod_cast Nat.one_le_pow _ _ (by omega))
        _ ≤ (2 : ℚ) ^ (b - 127) := by exact_mod_cast Nat.one_le_pow _ _ (by omega)
    · push_neg at ha hb
      simp only [show ¬(a ≥ 127) from by omega, show ¬(b ≥ 127) from by omega, ↓reduceIte]
      rw [one_div, one_div]
...
theorem ebe_normal (x : F32) (h : x.biasedExp.val ≠ 0) :
    x.effectiveBinaryExp = (x.biasedExp.val : Int) - 150 := by
  unfold effectiveBinaryExp expBias mantBits; split <;> omega

theorem esig_ge_23 (x : F32) (h : x.biasedExp.val ≥ 1) :
    (x.effectiveSignificand : ℚ) ≥ 2^23 := by
  have : x.effectiveSignificand ≥ 2^23 := by
    unfold effectiveSignificand; simp only [mantBits]; split <;> omega
  exact_mod_cast this

theorem esig_lt_24 (x : F32) :
    (x.effectiveSignificand : ℚ) < 2^24 := by
  suffices h : x.effectiveSignificand < 2^24 from by exact_mod_cast h
  unfold effectiveSignificand; simp only [mantBits]
  have hm := x.mantissa.isLt
  split
  · linarith [show (2:Nat)^23 < 2^24 from by norm_num]
  · linarith [show (2:Nat)^23 + 2^23 = 2^24 from by norm_num]

theorem esig_lt_23_sub (x : F32) (h : x.biasedExp.val = 0) :
    (x.effectiveSignificand : ℚ) < 2^23 := by
  suffices hh : x.effectiveSignificand < 2^23 by exact_mod_cast hh
  unfold effectiveSignificand; simp [h]

theorem lower_bound (x : F32) (hfin : x.isFinite) (hbexp : x.biasedExp.val ≥ 1) :
    threshQ x.biasedExp.val ≤ |x.toRat| := by
  rw [toRat_abs x hfin]
  have hesig := esig_ge_23 x hbexp
  have hbne : x.biasedExp.val ≠ 0 := by omega
  have hebe := ebe_normal x hbne
  by_cases hexp : x.effectiveBinaryExp ≥ 0
  · simp only [if_pos hexp]
    have hbge : x.biasedExp.val ≥ 150 := by rw [hebe] at hexp; omega
    have htonat : x.effectiveBinaryExp.toNat = x.biasedExp.val - 150 := by
      rw [hebe]; omega
    rw [htonat]
    unfold threshQ; simp only [show x.biasedExp.val ≥ 127 from by omega, ↓reduceIte]
    calc (2:ℚ)^(x.biasedExp.val - 127)
        = 2^23 * 2^(x.biasedExp.val - 150) := by rw [← pow_add]; congr 1; omega
      _ ≤ ↑x.effectiveSignificand * 2^(x.biasedExp.val - 150) :=
          mul_le_mul_of_nonneg_right hesig (by positivity)
  · push_neg at hexp
    simp only [if_neg (show ¬(x.effectiveBinaryExp ≥ 0) from by omega)]
    have hblt : x.biasedExp.val < 150 := by rw [hebe] at hexp; omega
    have htonat : (-x.effectiveBinaryExp).toNat = 150 - x.biasedExp.val := by
      rw [hebe]; omega
    rw [htonat]
    unfold threshQ
    by_cases hge127 : x.biasedExp.val ≥ 127
    · simp only [show x.biasedExp.val ≥ 127 from hge127, ↓reduceIte]
      rw [le_div_iff₀ (by positivity : (0:ℚ) < 2^(150 - x.biasedExp.val))]
      calc (2:ℚ)^(x.biasedExp.val - 127) * 2^(150 - x.biasedExp.val)
          = 2^23 := by rw [← pow_add]; congr 1; omega
        _ ≤ ↑x.effectiveSignificand := hesig
    · push_neg at hge127
      simp only [show ¬(x.biasedExp.val ≥ 127) from by omega, ↓reduceIte]
      rw [div_le_div_iff₀ (by positivity) (by positivity), one_mul]
      calc (2:ℚ)^(150 - x.biasedExp.val)
          = 2^23 * 2^(127 - x.biasedExp.val) := by rw [← pow_add]; congr 1; omega
        _ ≤ ↑x.effectiveSignificand * 2^(127 - x.biasedExp.val) :=
            mul_le_mul_of_nonneg_right hesig (by positivity)

theorem upper_bound_sub (x : F32) (hfin : x.isFinite)
    (hbexp : x.biasedExp.val = 0) (_hne : x.toRat ≠ 0) :
    |x.toRat| < threshQ 1 := by
  rw [toRat_abs x hfin]
  have hesig := esig_lt_23_sub x hbexp
  have hebe : x.effectiveBinaryExp = -149 := by
    unfold effectiveBinaryExp expBias mantBits; simp [hbexp]
  simp only [hebe, show ¬((-149 : Int) ≥ 0) from by omega, ↓reduceIte]
  have : (-(-149 : Int)).toNat = 149 := by omega
  rw [this]
  unfold threshQ; simp only [show ¬(1 ≥ 127) from by omega, ↓reduceIte,
                              show 127 - 1 = 126 from by omega]
  rw [div_lt_div_iff₀ (by positivity) (by positivity), one_mul]
  calc ↑x.effectiveSignificand * (2:ℚ)^126
      < 2^23 * 2^126 := mul_lt_mul_of_pos_right hesig (by positivity)
    _ = 2^149 := by rw [← pow_add]

theorem upper_bound_norm (x : F32) (hfin : x.isFinite) (hbexp : x.biasedExp.val ≥ 1) :
    |x.toRat| < threshQ (x.biasedExp.val + 1) := by
  rw [toRat_abs x hfin]
  have hesig := esig_lt_24 x
  have hbne : x.biasedExp.val ≠ 0 := by omega
  ...
  · simp only [if_pos hexp]
    ...
    have htonat : x.effectiveBinaryExp.toNat = x.biasedExp.val - 150 := by
      rw [hebe]; omega
    rw [htonat]
    unfold threshQ; simp only [show x.biasedExp.val + 1 ≥ 127 from by omega, ↓reduceIte]
    calc ↑x.effectiveSignificand * (2:ℚ)^(x.biasedExp.val - 150)
        < 2^24 * 2^(x.biasedExp.val - 150) := mul_lt_mul_of_pos_right hesig (by positivity)
      _ = 2^(x.biasedExp.val + 1 - 127) := by rw [← pow_add]; congr 1; omega
  · push_neg at hexp
    simp only [if_neg (show ¬(x.effectiveBinaryExp ≥ 0) from by omega)]
    have hblt : x.biasedExp.val < 150 := by rw [hebe] at hexp; omega
    have htonat : (-x.effectiveBinaryExp).toNat = 150 - x.biasedExp.val := by
    ...
    rw [htonat]
    unfold threshQ
    by_cases hge126 : x.biasedExp.val + 1 ≥ 127
    · simp only [show x.biasedExp.val + 1 ≥ 127 from hge126, ↓reduceIte]
      rw [div_lt_iff₀ (by positivity : (0:ℚ) < 2^(150 - x.biasedExp.val))]
      calc (↑x.effectiveSignificand : ℚ)
          < 2^24 := hesig
        _ = 2^(x.biasedExp.val + 1 - 127) * 2^(150 - x.biasedExp.val) := by
            rw [← pow_add]; congr 1; omega
    · push_neg at hge126
      simp only [show ¬(x.biasedExp.val + 1 ≥ 127) from by omega, ↓reduceIte]
      rw [div_lt_div_iff₀ (by positivity) (by positivity), one_mul]
      calc ↑x.effectiveSignificand * (2:ℚ)^(127 - (x.biasedExp.val + 1))
          < 2^24 * 2^(127 - (x.biasedExp.val + 1)) :=
            mul_lt_mul_of_pos_right hesig (by positivity)
        _ = 2^(150 - x.biasedExp.val) := by rw [← pow_add]; congr 1; omega

/-- findBiasedExp correctly recovers the biased exponent. -/
theorem findBiasedExp_correct (x : F32) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
...
theorem binExp_eq (x : F32) :
    (if x.biasedExp.val = 0 then (-149 : Int) else (x.biasedExp.val : Int) - 150) =
...
private theorem sigExact_eq (x : F32) (hfin : x.isFinite) (_hne : x.toRat ≠ 0) :
    let binExp : Int := if x.biasedExp.val = 0 then -149 else (x.biasedExp.val : Int) - 150
...
theorem esig_subnormal (x : F32) (h : x.biasedExp.val = 0) :
    x.effectiveSignificand = x.mantissa.val := by
  unfold effectiveSignificand; simp [h]

theorem esig_normal (x : F32) (h : x.biasedExp.val ≠ 0) :
    x.effectiveSignificand = 2^23 + x.mantissa.val := by
  unfold effectiveSignificand; simp [h, mantBits]

/-- roundSignificand recovers the mantissa exactly with no bump. -/
theorem roundSignificand_exact (x : F32) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
...
theorem round_nearest_nonzero (x : F32) (hfin : x.isFinite) (hne : x.toRat ≠ 0) :
    ...
    · split
      · next hmk =>
        ...
        simp only [show x.biasedExp.val < 255 from hexp_lt,
                   show x.mantissa.val < 2^23 from x.mantissa.isLt, dite_true] at hmk
        rw [hdec] at hmk
        simp only [Option.some.injEq] at hmk
        rw [← hmk]
      · next hmk =>
        exfalso; unfold mkFinite at hmk
        simp [show x.biasedExp.val < 255 from hexp_lt] at hmk
